...
def run(im, multi=False, instruction=0):
    im_disp = im.copy()
    im_draw = im.copy()
    window_name = "Select box"
    if instruction == 0:
        window_name = "Select objects to be tracked here."
    else:
        window_name = "Box baseline to free throw lane."
    ...
    def callback(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            if multi is False and len(pts_2) == 1:
                print("Delete the previously selected object" +
                      " using key `d` to mark a new location.")
                return
            run.mouse_down = True
            if instruction == 1:
                x, y = 0, 0
