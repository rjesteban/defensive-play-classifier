...
def animate(n):
    ...
    # i could change the size of the ball according to its height, but chose
...
player_xy = np.array([np.array(x[5][1:])[:, 1:4] for x in data['events'][
                     event_num]['moments']])  # create matrix of player data
...
ani.save('Event_%d.mp4' % (search_id), dpi=100, fps=25)
plt.close('all')
